...
class RowMaskedLinear(nn.Linear):
    def __init__(
        ...
        super(RowMaskedLinear, self).__init__(in_features=in_features, out_features=out_features, bias=bias)
        self.pruning_method = pruning_method
        self.mask_scale = mask_scale
        self.mask_init = mask_init
...
class RowTopKBinarizer(autograd.Function):
    """
    Row Top-k Binarizer.
    Computes a binary mask M from a real value matrix S such that M_{i,j} = 1 if and only if the row containing S_{i,j}
    is among the k% rows with the highest norm.
    """
    @staticmethod
    def forward(ctx, inputs: torch.tensor, threshold: float):
        # Get the subnetwork by sorting the row norms and using the top threshold %
        if not isinstance(threshold, float):
            threshold = threshold[0]

        # Calculate the norm of each row
        row_norms = inputs.norm(dim=1)  

        # Get the top k% indices based on the row norms
        _, idx = row_norms.sort(descending=True)
        j = int(threshold * len(row_norms))

        # Create a mask based on the selected rows
